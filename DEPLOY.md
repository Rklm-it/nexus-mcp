...
```
Ставит `python3-light` с модулями (~10 МБ; точный размер установщик считает по
спискам opkg и печатает). Если на флеш не влезает, python3 ставится в ОЗУ
(`/tmp/nexus-py`, нужно его размер + 40 МБ свободной памяти): флеш не
...
**Сквозная через sing-box.** Если на роутере уже стоит sing-box (его ставит
podkop), пробник берёт его, а не xray: ничего не качается, хватает ~48 МБ
свободной ОЗУ. Профиль для sing-box хаб перекладывает из того же xray-конфига,
что получает клиент. VLESS (ws/httpupgrade/grpc, tls/reality), SS2022 и
Hysteria2 проверяются так; xhttp у sing-box нет — такие строки только через xray.

**Строки за Cloudflare** (VLESS WS · CF) хаб привязывает к ноде по имени в домене
(`eng41s2.pablo.stream` → `eng41s2`). Зелёный на первой ступени у них значит
«Cloudflare доступен с этой сети» — дошло ли до ноды, говорит только сквозная.

**Обновление пробника.** Пробник от 1.2.0 обновляется сам: хаб при опросе видит
версию старее своей и присылает свой `probe.py`, пробник заменяет файл и
перезапускается. Хватает обновить хаб. Выключить: `NEXUS_PROBE_NO_UPDATE=1`.
Пробник 1.1.0 так не умеет — один раз перезапустите команду установки.
